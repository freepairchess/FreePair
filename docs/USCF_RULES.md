...
- **Phase C** — engine gap closure, one branch per **planned** row, each with
...
| [22B](#rule-22b) | Full-point byes (defers to 28L) | **enforced** | `UscfPairer.PairRoundN` bye picker | `28L` (over-general → `28L2`) |
...
| [27A1](#rule-27a1) | Avoid players meeting twice (highest priority) | **enforced** | `UscfPairer.HasPlayed` / `IsForbiddenPair` | `29B` (wrong → `27A1`) |
| [27A2](#rule-27a2) | Equal scores | **enforced** | `UscfPairer.PairRoundN` score-group enumeration | — |
| [27A3](#rule-27a3) | Upper half vs. lower half | **enforced** | `UscfPairer.PairPool` SLIDE | `28C` (wrong → `27A3` / `28J`) |
| [27A4](#rule-27a4) | Equalizing colors | **enforced** | `UscfPairer.TopGetsWhite` + `TryReduceColorConflicts` | `29D1` (wrong → `29E4 step 2`) |
| [27A5](#rule-27a5) | Alternating colors | **enforced** | `UscfPairer.TopGetsWhite` step 3 | `29D2` (wrong → `29E4 step 3`) |
...
| [28J](#rule-28j) | The first round | **enforced** | `UscfPairer.PairRoundOne` | `28C` (wrong → `28J`) |
| [28K](#rule-28k) | Late entrants | **TD discretion** | `AppendPlayer` + late-entrant numbering | — |
| [28L](#rule-28l) | Full-point byes (chapter umbrella) | **enforced** | `UscfPairer.PairRoundN` bye picker | `28L` (over-general → `28L2`) |
| [28L1](#rule-28l1) | Explanation and display | **enforced** | `Round.Byes` + wall-chart rendering | — |
| [28L2](#rule-28l2) | Determination (who gets the bye) | **enforced** | `UscfPairer.PairRoundN` bye picker | `28L` (over-general → `28L2`) |
...
| [28O1](#rule-28o1) | Computer wall charts | **enforced** | `WallChartViewModel` + `PdfReportBuilder` | — |
...
| [29A](#rule-29a) | Score groups and rank | **enforced** | `UscfPairer.PairRoundN` group enumeration | — |
| [29B](#rule-29b) | Order of pairing score groups | **enforced** | `UscfPairer.PairRoundN` descending loop | — |
| [29C](#rule-29c) | Method of pairing score groups | **enforced** | `UscfPairer.PairPool` | — |
...
| [29E2](#rule-29e2) | First-round colors | **enforced** | `UscfPairer.PairRoundOne` initial-colour rule | `29E1` (wrong → `29E2`) |
| [29E3](#rule-29e3) | Due colors in succeeding rounds | **enforced** | `TopGetsWhite` due-colour logic | — |
| [29E3a](#rule-29e3a) | Due colors defined | **enforced** | `TopGetsWhite` + `PreferredColor` | — |
| [29E4](#rule-29e4) | Equalization, alternation, and priority of color | **enforced** | `TopGetsWhite` steps 1–5 | `29D1` / `29D2` / `29D` (wrong → `29E4`) |
...
- <a id="rule-29d1a"></a>**(a) Default.** The **lowest-rated rated** player (not unrated)
  ...
  are acceptable.
- <a id="rule-29d1b"></a>**(b) When (a) fails.** Try the next-lowest-rated player as the
...
**Status.** **TD discretion** — implemented but **gated off by default** (no corpus
